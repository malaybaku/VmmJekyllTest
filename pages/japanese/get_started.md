...
PC上の`.vrm`ファイルを選択し、キャラクター表示ウィンドウに表示される規約を確認します。
...
ブラウザ上でアプリ連携を求められるため、許可を行い、認可コードを表示します。

表示された認可コードをコピーし、VMagicMirrorの入力欄にペーストしてログインします。

その後、自分でアップロードしたモデル、「いいね」したモデル、ピックアップされたモデルのいずれかを選択し、規約を確認してロードします。
...
この時点でキーボードやタッチパッドの位置、視点がキャラクターに合わない場合、`キャラ体格で補正`ボタンをクリックすると補正できます。
...
ここで補正した視点などは、後で紹介する方法でより細かく調整できます。
...
キャラクターがうつむきがちになる場合は、少し下を向いて`姿勢・表情を補正`ボタンをクリックすると、キャラが上を向きやすくなります。反対に、キャラクターが上を向きすぎる場合は、上を剥いて`姿勢・表情を補正`ボタンをクリックすることで、キャラが下を向きやすくなります。
...
キャラクターを見失ってしまった場合は`位置をリセット`ボタンでリセットして下さい。

あるいは、`背景を透過`をオフにしてウィンドウの表示を確認します。
